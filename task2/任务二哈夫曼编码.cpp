...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]!=0)//*补充的代码 
        {
            arr_small[count2++] = arr_big[i];	//记录该字母在字符串中出现的次数并放入arr_small中 *
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //该节点为叶子节点 /*补充判断模块注释*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //确保节点为未被添加到哈夫曼树的叶节点/*补充判断模块注释（说明作用）*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //遍历所有未被添加到哈夫曼树节点并找出这其中权值最小节点 /*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    {
        if(huffman[i1][3] == -1 && huffman[i1][0] == -1)  //确保节点为未被添加到哈夫曼树的叶节点
		{
		break;
		}
        if(i1!=ii[0])
		{
		    if(huffman[i1][0] == -1 && huffman[i1][3] < min2)  //找出没有父节点且权重次小的节点
		  	{
            	min2 = huffman[i1][3];
            	ii[1] = i1;
			}
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //如果没有父节点则返回长度为零  /*补充判断模块注释*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
	...
	while(father != -1)
    {
        ...
        father =huffman[current][0]; /*自行填充代码*/		//同理（当前结点更新后），更新当前结点的父亲结点
        start++;		       /*自行填充代码*/		//更新编码位置
    }
    code[start]='\0';   //编码结束符
